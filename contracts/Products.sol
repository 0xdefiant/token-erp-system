...
contract Products {
    ...
    modifier productExists(string memory id_) {
        require(bytes(product[id_].barcodeId).length != 0, "Product does not exist");
        _;
    }

    /**
     * @notice To check if product does not exist
     * @param id_ product barcode ID
     */
    modifier productNotExists(string memory id_) {
        require(bytes(product[id_].barcodeId).length == 0, "Product already exists");
        _;
    }
